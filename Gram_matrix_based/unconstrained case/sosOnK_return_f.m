...
% function [ prg, tk ,varargout] = sosOnK( prg, p, x, h, d )
% % keyboard
% if isa(x,'msspoly')
...
% %     keyboard
% if ~isempty(S)
%     [ prg, tk ] = prg.withSOS( p - S'*h ); % ensure that p-S'h is SOS on K 
% else
...
% %     varargout{1} = deg(p - S'*h, x);
% end


function [ prg, tk ,f,varargout] = sosOnK_return_f( prg, p, x, h, d )
    m = size( h, 1 ); % total number of h's
    S = msspoly( zeros( m, 1 ) ); % create this many multiplier variables

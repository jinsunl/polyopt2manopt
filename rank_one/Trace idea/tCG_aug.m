 5 function [eta, Heta, inner_it, stop_tCG] ...
                 = tCG_aug(problem, x, grad, eta, Delta, options, storedb, key)
...
% 
% JINSUN: DO NOT TOUCH ANY CODE W.R.T. options.useRand==1
...
theta = options.theta;
kappa = options.kappa;
constrnum = problem.constrnum;

% Delta = sqrt((Delta*8)^2 + constrnum)/8;
% Delta = sqrt(length(x)+constrnum)/8;
% Delta = sqrt(length(x)-constrnum)/8;

% eta = [eta;zeros(constrnum,1)];
gradC = 2*problem.stackA'*RepDiagFun(x,constrnum);
eta = [eta;2*problem.N*gradC'*eta];

x = [x;zeros(constrnum,1)];
grad = [grad;zeros(constrnum,1)];

if ~options.useRand % and therefore, eta == 0
    Heta = [problem.M.zerovec(x);zeros(constrnum,1)];
    r = grad;
    e_Pe = 0;
...
if ~options.useRand
    z = getPrecon(problem, x, r, storedb, key);  % we actually don't need this, since we don't have problem.precon or problem.sqrtprecon, so z = r anyway.
else
    z = r;
end
...
if ~options.useRand % therefore eta == 0
    model_value = 0;
else
...
%     Hdelta = getHessian(problem, x, delta, storedb, key);
    Hdelta = getHessian_aug(problem, x, delta, storedb, key);
    ...
    % Precondition the residual.
    if ~options.useRand
        z = getPrecon(problem, x, r, storedb, key); % again, this is unnecessary actually
    else
        z = r;
    end
...
eta = eta(1:end-constrnum);
Heta = Heta(1:end-constrnum);

function [ F,A,C,Mandim,B,b ] = pre_process_new( f,h,x,i,dim )
...
%   Detailed explanation goes here
    ...
    file_name = strcat('./magicA_lib/magicA',num2str(i),'_',num2str(dim),'.mat');
    ...
    %generate random F
    F = randn(length(zero),length(zero));
...
if nh>0,    
    ...
    % manage the size of matrices in A
    A_row_num = size(A{1},1);
    if exist('A','var') && A_row_num~=size(zero,2),
        for ii = 1:length(A),
            A{ii} = blkdiag(A{ii},zeros(size(zero,2)-A_row_num ));
        end
    end
    ...
    for ii = 1:nh,
        ...
        for iii = 1:length(monoh),
            ...
            A{count} = A{count} + temp1'*temp2*ch(iii);
        end
        C{count} = c;
        C{count}(count) = 1;
        count = count+1;
